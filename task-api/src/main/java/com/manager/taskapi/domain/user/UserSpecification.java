...
public class UserSpecification {
    ...
    public Specification<User> nameLikeIfNotNull(String name) {
        return name != null ? (root, query, builder) -> builder.like(builder.lower(root.get("name")), name) : alwaysTrue();
    }

    public Specification<User> emailLikeIfNotNull(String email) {
        return email != null ? (root, query, builder) -> builder.like(builder.lower(root.get("email")), email) : alwaysTrue();
    }
